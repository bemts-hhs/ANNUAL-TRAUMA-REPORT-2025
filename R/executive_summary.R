...
incident_count_plot <- incident_count_years |>
  ...
  ggrepel::geom_text_repel(
    nudge_y = ifelse(incident_count_years$Year == 2021, -300, 400),
    segment.color = "transparent",
    seed = 8182024,
    direction = "both",
    color = "darkblue",
...
# Compute transfer delay summaries for 2024–2025 using imputed Length_of_Stay ----
transfer_delays_2025 <- trauma_data_clean |>

  # Filter to only cases from 2024–2025 that were acute interfacility transfers ----
  dplyr::filter(Year %in% 2024:2025, Acute_Transfer_Out == "Yes") |>
...
gender_group_tbl <-
  ...
  gt::row_group_order(groups = c("2024", "2025")) |>
  gt::tab_footnote(
...
age_group <- trauma_data_clean |>
  ...
  dplyr::mutate(
    ...
    trend = ifelse(Year == 2021, "start", trend),
    .by = Age_Range
  ) |>
  dplyr::mutate(
...
age_group_plot_df <- age_group |>
  dplyr::filter(Year %in% 2022:2025, Age_Range != "Missing") |>
  dplyr::mutate(
...
age_group_plot <- age_group_plot_df |>
  ...
  ggplot2::labs(
    ...
    subtitle = "Patients Seen at a Trauma Center | Data: Iowa Trauma Registry 2022-2025",
...
leading_causes_plot <- leading_causes_years |>
  ...
  ggplot2::labs(
    ...
    subtitle = "Proportion of Total Injury Events is Column Height and Label | Years: 2019-2025",
...
mvc_injury_transpose <-
  ...
  purrr::set_names(nm = c("Category", 2021:2025)) |>
  dplyr::mutate(
    Category = c(
...
mvc_injury_table <-
  ...
  gt::tab_header(
    ...
    subtitle = "Patients Seen at a Trauma Center | Data: Iowa Trauma Registry 2021-2025"
  ) |>
...
reinjury_stat_tbl_object <- reinjury_stats_injuries |>
  ...
  t() |>
  as.data.frame() |>
  purrr::set_names(nm = c(2021:2025)) |>
  tibble::rownames_to_column(var = "Category") |>
  dplyr::filter(Category != "Year") |>
  dplyr::rowwise() |>
  dplyr::mutate(
    `2021-2025 Trend` = list(c(`2021`, `2022`, `2023`, `2024`, `2025`))
  ) |>
  dplyr::ungroup() |>
  dplyr::select(-c(`2021`, `2022`, `2023`))
...
intentionality_of_injury_pivot <- intentionality_of_injury |>
  ...
  tidyr::replace_na(list(`2021` = 0)) |>
  dplyr::mutate(
    `2021-2025 Trend` = list(c(`2021`, `2022`, `2023`, `2024`, `2025`)),
    .by = Category
  ) |>
  dplyr::select(-c(`2021`:`2023`))
...
intentionality_of_injury_tbl <-
  ...
  gt::tab_header(
    ...
    subtitle = "Data: Iowa Trauma Registry 2021-2025"
  ) |>
...
trauma_activation_cases_tbl <-
  ...
  gt::tab_spanner(label = "2022", columns = matches("_2022")) |>
  gt::tab_spanner(label = "2024", columns = matches("_2024")) |>
  gt::tab_spanner(label = "2025", columns = matches("_2025")) |>
